...
border = border.sort_values("area", ascending=False)\
               .iloc[[0]].drop(columns="area")
...
print("▶ Pobieranie zabudowy mieszkaniowej...")
...
print("▶ Obliczanie dostępności generatorów...")
...
def score_point(point, minutes):
    ...
    # dystans do konkurencji
    dists = [
        nx.shortest_path_length(G, node, ln, weight="length")
        for ln in locker_nodes
    ]
...
for _, row in candidates.sort_values(
    ...
    if all(
        row.geometry.distance(p) > MIN_DIST_BETWEEN
        for p in selected
    ):
...
print("✔ ANALIZA ZAKOŃCZONA — WERSJA FINALNA")
